...
| **文字 / 背景顏色** | 色彩選擇器 |
| **背景透明度** | 0%（無背景）~ 100% |
| **水平邊距 (X)** | 水平方向相對百分比邊距（0% ~ 10%，預設 3%） |
| **垂直邊距 (Y)** | 垂直方向相對百分比邊距（0% ~ 10%，預設 3%） |
| **檔名後綴** | 自訂輸出檔名後綴，留空則自動產生當天日期 |
| **檔名命名模式** | 使用 `{name}`、`{suffix}`、`{token}` 組合，預設 `{name}_{suffix}_{token}` |
| **批次 Token 長度** | 4~12 字元（預設 6），防止批次檔名衝突 |
| **原始照片 TAG** | 套用後自動為原始照片新增指定 TAG（勾選啟用） |
| **新生成照片 TAG** | 可繼承原始照片 TAG，並額外附加自訂 TAG |

**輸出格式：**
- 檔名（預設模式）：`原始檔名_後綴_a1b2c3.jpg`
- 備註欄：`[時間戳記:後綴] 原始檔案：原始檔名.jpg`
- 檔名長度上限：基底名稱最多 200 字元（自動截斷 `{name}` 保護 `{suffix}` 與 `{token}`）
